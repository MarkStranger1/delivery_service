...
import DishesData from "../../shared/dishesData.json"
import TypesData from "../../shared/typesData.json"
import "./style.scss"
...
export const HomePage = () => {
    ...
    const [productsType, setProductType] = useState<Array<TypeOfDish> | null>(null)
    ...
    const applyFilter = (data: Array<Dish>) => {
        if (selectedProdType)
            return data.filter(dish => dish.type.slug === selectedProdType.slug);
        return data;
    }

    const changeTypeHandler = (type: TypeOfDish) => {
        setChangeData(-1);
        setTimeout(() => {
            setChangeData(1);
            ...
            <div className="select-product-type">
                {productsType
                    ...
                    && productsType.map((type, index) => {
                        return <>
                            <button
                                onClick={() => changeTypeHandler(productsType.find(type => type.id === (index + 1))!)}
